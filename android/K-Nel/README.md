# K-Nel Alioth
![Alt text](/android/K-Nel/logo.png?raw=true "A Custom Kernel Base For Redmi K40 / Xiaomi 11X / POCO F3")
* 本仓库为 K-Nel Alioth 的仓库，仅存储为 红米K40 / 小米11X / POCO F3 编译的 K-Nel 内核
* 内核维护者（K-Nel Developer）：
* [科家（KNKJ55）](http://knkj55.github.io/about/kejia/)
* 编译环境：Github Actions - Ubuntu 24.04
* 工具链：Google LLVM&Clang
* 测试机型：红米K40
* 如果您有需要，可以使用本仓库的源码重新编译 K-Nel
* PS: 刷机有风险，搞机需谨慎！
---
* 该内核基于 github@Live-House/android_kernel_xiaomi_sm8250 修改
* 感谢 github@bxySo
